...
async def export_sessions(
    ...
    csv_content = (
        await exporter.export_multiple_sessions_admin(
            list(session_ids), db
        )
    )
...
async def export_selected_sessions(
    ...
    for s in sessions:
        if s.ended_at is None:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=(
                    f"Session {s.id} is still active."
                    " Only ended allowed."
                ),
            )

    valid_ids = [
        s.id for s in sessions if s.deleted_at is None
    ]
    if not valid_ids:
        return Response(
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": (
                "attachment;"
                " filename=selected_sessions.csv"
            )
        },
    )
...
@router.get(
    "/admin/user-conversations/{user_id}/export"
)
