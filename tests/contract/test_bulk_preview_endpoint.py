...
async def admin(async_session):
    user = User(username="prev_admin", nickname="관리자", role="admin")
...
def _csv_file(content: str):
    return {"file": ("test.csv", content.encode("utf-8"), "text/csv")}


class TestBulkPreview:
    @pytest.mark.asyncio
    async def test_valid_csv_returns_preview(self, async_client, admin, group):
    ...
    @pytest.mark.asyncio
    async def test_invalid_csv_returns_400(self, async_client, admin):
        await _login(async_client, admin.username)
        resp = await async_client.post(
            "/admin/users/bulk/preview",
