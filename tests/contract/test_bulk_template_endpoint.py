...
async def admin(async_session):
    user = User(username="tmpl_admin", nickname="관리자", role="admin")
...
async def teacher(async_session):
    user = User(username="tmpl_teacher", nickname="교사", role="teacher")
    user.set_password("test1234")
    async_session.add(user)
    await async_session.flush()
    return user
...
class TestBulkTemplateDownload:
    ...
    async def test_admin_can_download(self, async_client, admin):
        ...
        resp = await async_client.get("/admin/users/bulk/template")
