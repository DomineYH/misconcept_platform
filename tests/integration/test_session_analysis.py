...
class TestSessionAnalysisWorkflow:
    ...
    async def test_complete_dialogue_triggers_question_analysis(
        self, test_client: TestClient, async_db_session: AsyncSession
    ):
        """
        Verify complete dialogue → end session → QuestionAnalysis created.
        ...
        4. Ends session
        5. Verify QuestionAnalysis records created for each teacher msg
        6. Verify SessionSummary created with correct distribution
        """
        # Step 1: Login
        ...
        teacher_messages = [
            "What is photosynthesis?",  # High-leverage (open question)
            "Can you explain more?",  # Medium
            "Is it related to sunlight?",  # Low-leverage (yes/no)
        ]
    ...
    async def test_session_with_no_teacher_messages_creates_empty_analysis(
        ...
        """Verify session with no teacher messages creates empty summary."""
        ...
        # Verify SessionSummary created with empty distribution
        result = await async_db_session.execute(
            select(SessionSummary).where(
                SessionSummary.session_id == session_id
            )
    ...
    async def test_question_analysis_labels_match_framework(
        ...
        """Verify QuestionAnalysis labels match AnalysisFramework."""
        # Login and create session
        login_response = test_client.post(
            "/login",
        ...
        test_client.post(
            f"/sessions/{session_id}/messages",
            json={"content": "Explain your thinking process."},
            cookies=cookies,
        )
